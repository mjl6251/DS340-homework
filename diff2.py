...
kmeans = KMeans(n_clusters=6, random_state=0, init='k-means++', max_iter=100)
            ...
            for wd in res:
                ...
                if len(wd)>1:
                    try:
                        ...
                        continue
    res = [x for x in res if x not in def_words]
    print(res)
    return None
    #return HttpResponse(json.dumps(ret,ensure_ascii=False),content_type="application/json,charset=utf-8")
...
def main():
    vars = []
    vars.append("a road where cars go very quickly without stopping")
    vars.append("when somebody gives something to you and afterwards you have it")
    vars.append("a command, password, or signal.")
    vars.append("a fixed luminous point in the night sky which is a large, remote incandescent body like the sun.")
    vars.append("a human being regarded as an individual.")
    vars.append("space that can be occupied or where something can be done, especially viewed in terms of whether there is enough.")
    vars.append("an opening in the wall or roof of a building or vehicle that is fitted with glass or other transparent material in a frame to admit light or air and allow people to see out.")
    vars.append("a hinged, sliding, or revolving barrier at the entrance to a building, room, or vehicle, or in the framework of a cupboard.")
    vars.append("an intense feeling of deep affection.")
    vars.append("the state of being free from illness or injury.")
    #var = input("Please enterr your definition: ")
    #EnglishRD(var)
    for var in vars:
        print(var)
        EnglishRD(var)
